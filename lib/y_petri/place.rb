#encoding: utf-8
# Actually, YPetri::Cell is a Petri net place.

module YPetri
  # FIXME: Most of this class in its current form is not tested
  class Place
    ...
    def arcs; upstream_arcs | downstream_arcs end
    ...
    # Adding tokens
    def add( amount_of_tokens ); @marking += amount_of_tokens end

    # Subtracting tokens
    def subtract( amount_of_tokens); @marking -= amount_of_tokens end

    # Resets marking back to default marking
    def reset_marking; @marking = @default_marking end
    ...
    def fire_upstream!
      @upstream_arcs.each &:fire! end
    alias :fire! :fire_upstream!

    # Fires whole upstream portion of the net.
    def fire_upstream_recursively
      # so far no concerns about infinite loops
      # LATER: This as global hash { place => fire_list }
    ...
    # #inspect
    def inspect; "YPetri::Place instance with marking %s" % marking end

    # #to_s
    def to_s
      "#{@name ? @name : 'unnamed place'} [ %s ]" % marking
    end
  end # class Place
end # module YPetri
